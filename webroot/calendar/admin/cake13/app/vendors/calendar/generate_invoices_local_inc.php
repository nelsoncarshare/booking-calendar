...
function generate_invoice_for_user_or_group($query_month, $query_year, $invoicable_id, $transID, $invoiceNumber, $previousOwing, $paymentsMade, $billing, $invoiceMemoText, &$outErrors){
	...
	$pst_invoice_row = getPSTInvoiceRow($totalDistance, $billing, $subtotals);
	$outData['pst_row'] = $pst_invoice_row;
	
	$rentalTaxRow = getRentalTaxRow($eventsRows, $billing, $subtotals);
	...
	$carbonOffsetRow = getCarbonOffsetTaxInvoiceRow($totalDistance, $billing, $subtotals);
	$outData['carbon_offset_row'] = $carbonOffsetRow;
	
	$selfInsuranceRow = getSelfInsuranceInvoiceRow($totalDistance, $billing, $subtotals);
	$outData['self_insurace_row'] = $selfInsuranceRow;
	
	$interestRow = getInterestInvoiceRow($previousOwing, $paymentsMade, $billing, $subtotals);
...
function skin_invoice_html($outData, &$outErrors){
	...
	$invDateArray = split("-", $outData['billing_info']["invoice_date"]);
	$invDueDateArray = split("-", $outData['billing_info']["invoice_due_date"]);
...
function getBookingInvoiceRow($totalDistance, $billing, $row, $plan, &$subtotals){
		...
		if ($row['admin_ignore_this_booking'] == 1){
			...
			$timeCharge = get_time_charge($numDaySlots, $numNightSlots, 
										$numExtraQuarterHoursFromCancelAndMod['day'], $numExtraQuarterHoursFromCancelAndMod['night'],
										$row['hourly_rate_low'], $row['hourly_rate_high'], $row['is_flat_daily_rate'], $row['daily_rate_low'], $row['daily_rate'], $row['rate_cutoff']);
...
function getHTMLLogoRow(){
	return tag("div", attributes("align='center'"),
			tag( "font", attributes("size='+2'"), "<b>Kootenay Carshare Cooperative</b><br/>"),
			"Suite 310, 622 Front St. &nbsp;&nbsp;Nelson, BC, &nbsp;&nbsp;V1L 4B7 &nbsp;&nbsp; info@carsharecoop.ca<br/><br/>"
		);
}
