	...
	while($row = $result->FetchRow($result)) {
		...
		if ($row['admin_ignore_this_booking'] == false){
			if ($row['expense_gas'] != 0){
				...
				$expenseRows[] = Array( "comment" => $row['admin_comment'], "value" => $row['expense_gas'], "acnt_code" => $row['acnt_code_gas'], "item" => "Gas" );
			}
			if ($row['expense_repair'] != 0){
				$subtotals['expenseTotal'] += round($row['expense_repair'], 2);
				$expenseRows[] = Array( "comment" => $row['admin_comment'], "value" => $row['expense_repair'], "acnt_code" => $row['acnt_code_repair'], "item" => "Repair" );
			}
			if ($row['expense_admin'] != 0){
				$subtotals['expenseTotal'] += round($row['expense_admin'], 2);
				$expenseRows[] = Array( "comment" => $row['admin_comment'], "value" => $row['expense_admin'], "acnt_code" => $row['acnt_code_admin'], "item" => "Administration" );
			}
			if ($row['expense_insurance'] != 0){
				$subtotals['expenseTotal'] += round($row['expense_insurance'], 2);
				$expenseRows[] = Array( "comment" => $row['admin_comment'], "value" => $row['expense_insurance'], "acnt_code" => $row['acnt_code_insurance'], "item" => "Insurance" );
			}		
			if ($row['expense_misc_1'] != 0){
				$subtotals['expenseTotal'] += round($row['expense_misc_1'], 2);
				$expenseRows[] = Array( "comment" => $row['admin_comment'], "value" => $row['expense_misc_1'], "acnt_code" => $row['acnt_code_misc_1'], "item" => "Misc 1" );			
			}		
			if ($row['expense_misc_2'] != 0){
				$subtotals['expenseTotal'] += round($row['expense_misc_2'], 2);
				$expenseRows[] = Array( "comment" => $row['admin_comment'], "value" => $row['expense_misc_2'], "acnt_code" => $row['acnt_code_misc_2'], "item" => "Misc 2" );			
			}
			if ($row['expense_misc_3'] != 0){
				$subtotals['expenseTotal'] += round($row['expense_misc_3'], 2);
				$expenseRows[] = Array( "comment" => $row['admin_comment'], "value" => $row['expense_misc_3'], "acnt_code" => $row['acnt_code_misc_3'], "item" => "Misc 3" );	
			}		
			if ($row['expense_misc_4'] != 0){
				$subtotals['expenseTotal'] += round($row['expense_misc_4'], 2);
				$expenseRows[] = Array( "comment" => $row['admin_comment'], "value" => $row['expense_misc_4'], "acnt_code" => $row['acnt_code_misc_4'], "item" => "Misc 4" );	
			}			
		}	
	}	
...
function getHTMLLogoRow(){
	...
	return tag("div", attributes("align='center'"),
			tag( "font", attributes("size='+2'"), "<b>" . $CARSHARE_NAME . "</b><br/>"),
			$CARSHARE_BILLING_ADDRESS
		);
}
